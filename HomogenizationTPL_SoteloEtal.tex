...
\section{Introduction}
Quantitative assessment of seismic reflectivity experiments is a valuable tool to characterize rock and fluid properties of underground structures of interest.
Interpretation of reflectivity data is particular relevant for the characterization of small-scale geological features such as thin beds. In this regard,
a layer is considered thin if the reflections  from the top and bottom interfaces can not be individually resolved by the predominant wavelength, in such a way that only their composite effect manifest as a single reflectivity response \cite{Widess1973}. For practical purposes, the onset of this effect is considered to occur when
the layer thickness 
is equal to a quarter of the dominant wavelength \cite{Widess1973, Kallweit1982}.
This threshold is known as the the tuning thickness, and is characterized by an initial constructive interference of the reflection signals from the top and bottom interfaces that becomes destructive as the layer increasingly thins \cite{Bakke1998, Hamlyn2014}.
...
and the monitoring of carbon sequestration \cite<e.g.,>{Williams2012,Zhang2013}. In general, proposed methodologies to estimate thin layer properties are largely developed in the frequency domain because it provides an increased resolution power  \cite<e.g.,>{Puryear2008,Rubino2009a,Zhang2013, Romdhane2014, Huang2016}, and in this context, they are mainly applied in a pure elastic framework. 

The poroelastic framework allows an accurate physical description of heterogenous porous rocks saturated with different fluids that are likely to be encountered in thin layers of interest. In particular, evidence suggests that heterogenoues porelastic media (PM)
exhibit a viscoelastic effective behavior regarding attenuation and velocity dispersion in the seismic frequency range. \cite<e.g.,>{Pride2004, Carcione2006}. This dispersive behavior is the consequence of an energy-dissipating phenomenon known as wave-induced fluid flow or WIFF \cite<e.g.,>{Muller2010} that occurs when a passing seismic wave generates pressure gradients between different parts of a PM 
that equilibrate by fluid flow.
For typical frequencies used in seismic experiments, WIFF occurs predominantly in the mesoscopicscale range \cite<e.g.,>{Pride2004, Muller2010}. This refers to  WIFF taking place  between  heterogeneities that are larger than the prevailing pore size  but smaller than the wavelength \cite<e.g.,>{Norris1993}. In this context, the substitution of a heterogeneous thin  poroelastic layer (TPL) by its homogenized viscoelastic representation can be deemed as an efficient technique to study its seismic reflectivity response. Indeed, published work
shows the applicability of the poroelastic-to-viscoelastic homogenization approach on reflecitivy studies of TPL. For instance, \citeA{Rubino2011} and \citeA{Rubino2011a} employ viscoelastic substitutes to represent thin sands with different patchy saturations of CO$_2$ to investigate the corresponding effects on zero-offset seismic data as well as on
the variation of amplitudes for different incident angles. Similarly,
\citeA{He2020} use viscoelastic substitutes of fractured thin layers to investigate their effect on P-wave amplitude variation with respect to incident angles. Moreover, \citeA{Jin2017} use an effective viscoelastic representation to replace a partially gas-saturated thin layer. This model is later used to estimate gas saturation and layer thickness from seismic amplitude variation with the incident angle.

The pioneering work of \citeA{White1975a} and \citeA{White1975} is one of the first to show the effective viscoelastic behavior of simple poroelastic composites saturated with gas and water. Specially, the model of periodically alternating poroelastic layers \cite{White1975} has been applied to represent TPL of interest and to study their  reflectivity response.  This modeling approach is convenient for testing the sensitivity of various poroelastic parameters because it allows the computation of the TPL effective moduli by analytical means \cite<e.g.,>{Carcione2006, Krzikalla2011}. For instance,  \citeA{Quintal2009} and \citeA{Quintal2011a} use this TPL representation to compare the reflection coefficients at normal incidence of viscoelastic substitutes of various TPL models characterized by alternating beds with different rock and fluid properties. Whereas, \citeA{He2020} utilize a particular version of this model to represent fractured TPL. In this description, the bed representing the fracture is much thinner and softer than the one representing the background rock \cite{Brajanovski2005,Kong2013}. In their study, they use viscoelastic substitutes of fractured TPL to examine the effect of various fluids and fracture physical properties on the variation of seismic amplitude with respect to the angle of incidence. A general assumption in the homogenization of a layered TPL is that the TPL should consist of a periodic heterogeneous structure, e.g., a pair of beds, that repeats a sufficient number of times so that its effective behavior is, for practical purposes, unaffected by the boundary conditions (BC) induced by the background embedding the TPL. However, a more realistic and flexible model to depict layered TPL for reflectivity analyses is to consider a TPL with a finite number of beds that present distinct rock and fluid properties. In such a case, BC induced by the embedding background are likely to affect the estimation of the effective moduli.
...
However, classical homogenization procedures are not readily applicable to this type of TPL models. At the same time, the development of suitable procedures has been largely unexplored.
...
As stated, traditional homogenization methods are not readily applicable to the case of a TPL with a few non-periodic beds. Nonetheless, for the sake of comparison, we first present the traditional procedure suggested by \citeA{Favino2020} 
...
%Hence, for the first step we use a sample $\Omega$ as shown in Figure \ref{fig.2}a.
...
%To further develop this point, Figures \ref{fig.4}a and b compare the pressure maps resulting from the compressional oscilatory  relaxation test for the homogenization procedures A and B, respectively for a frequency of 39.8 Hz that is within the dispersive region. Notice that the no-flow condition  imposed in procedure A at the top and bottom  boundaries of the sample induces a maximum pressure of around 47.6 KPa.  This pressure is 3.7 times higher than the maximum of around 12.8 KPa resulting from the periodic conditions applied by procedure B. Thus, the homogenization procedure A results in a higher average pressure on the sample that yields, consequently, stiffer moduli. The  difference between the estimated moduli is close to 1 GPa for this frequency. A similar effect can be associated with the homogenization procedure C that incorporates indirectly the no-flow condition by including part of the impermeable background in the sample.
%On the other hand,
%notice that discrepancies on the estimated moduli from procedures A and C due to different BC on displacement and tractions are unnoticeable for practical purposes. In this respect, procedure A imposes periodic BC on the top and bottom boundaries of the TPL sample. In contrast,  in procedure C, the presence of the background imposes indirectly a continuity of displacement and tractions on similar boundaries.
%
\begin{figure}[!ht]
\centering
...
\begin{figure}[!ht]
\centering
        \includegraphics[width= 140mm, height=110mm]{strain_sandfracw.eps}
