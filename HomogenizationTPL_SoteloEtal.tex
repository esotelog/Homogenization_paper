...
\title{Homogenization of heterogeneous porous thin layers for seismic reflectivity applications}
...
Classical poroelastic-to-viscoelastic homogenization  procedures of porous media with a deterministic arrangements of heterogeneities rely on the existence of a periodic heterogeneous structure, for example, an ensemble of beds that repeats sufficiently to ensure that its equivalent response is unaffected by external boundary conditions (BC). However the considered thin layer model does not present such periodicity and, therefore, the estimation of its equivalent viscoelastic moduli is inherently affected by the BC associated with the embedding background. Thus, we propose to account for the appropriate BC in the homogenization procedure. 
Classical homogenization methodologies perform oscillatory relaxation tests over a representative sample of the periodic structure, followed by the averaging of stress and strain components, which are then used to estimate the equivalent moduli. We adapt this procedure for our non-periodic thin layer model to incorporate the BC induced by the embedding background. To this end, we apply the oscillatory relaxation tests on a sample that includes part of the background while the averaging of stress and strain components is performed only over the thin layer section. To test the proposed method, we consider a thin layer embedded in impermeable half-spaces and that is composed by a stack of two porous sand beds, where the upper one is gas-saturated and the bottom one is water-saturated. Then, after estimating the thin layer equivalent moduli using the proposed homogenization procedure, we  calculate the P-wave reflectivities at the interface between the upper half-space and the thin layer equivalent representation. We compare these results against the ones obtained using the porous thin layer. 
Results show that the equivalent moduli obtained using the proposed methodology are able to closely reproduce the reflectivities of the porous thin layer in the seismic frequency range with errors below  4 \% . This methodology can be readily extended to porous thin layers presenting strongly heterogenous structures such as inclusions or fractures.
\end{abstract}
...
%the corresponding equivalent viscoelastic representation capable of reproducing the reflectivity response of such TPL model has not been widely investigated. 
...
%\caption{
%a)
%TPL model embedded in impermeable half-spaces $\Lambda_1$ and $\Lambda_2$. The TPL consists of two sand layers  $\Omega_{p1}$  and $\Omega_{p2}$ that are gas and fluid saturated, respectively. The light blue boxes represent the samples $\Omega$ and $\Omega_e$ used for the traditional and proposed homogenization procedures. b) Homogenized TPL (HTPL) embedded in the same half-spaces shown in (a). This HTPL represents the viscoelastic equivalent media obtained using either the proposed homogenization procedure (HTPL-bg) or the traditional one (HTPL-p). }
%\label{fig.2}
%\end{figure}

We assess the proposed homogenization methodology using a thin layer model of 1.2 m  of thickness that consists of two stack of poroelastic  sandstones beds B1 and B2, similar to the model presented in Figure \ref{fig.1}a. We consider that the upper bed B1 is CO$_2$-saturated whilst the lower one B2 is water-saturated. This thin layer is embedded 
in elastic-half spaces $\Lambda_1$ and $\Lambda_2$ (Figure \ref{fig.1}a and Table \ref{table.1}). To perform the evaluation, we follow the procedure described below: 
\begin{enumerate}
    ...
    \item Then, using these equivalent moduli, we calculate PP reflectivities  at the the interface between the upper half-space $\Lambda_1$ and the equivalent viscoelastic medium. Appendix B details the methodology for the PP reflectivity calculation.
...
On the other hand, to test whether the proposed method is independent of the size of the sampled background, we consider samples with background thicknesses equal to 0.12 m, 0.24 m and 0.48 m, respectively.
...
\begin{figure}[!ht]
\centering
        \includegraphics[width= 130mm, height=140mm]{strain_stress_sandUfrac2.eps}
...
\begin{figure}[!ht]
\centering
        \includegraphics[width= 130mm, height=140mm]{pressureU_kbg.eps}
...
Next, we examine the possible energy partition at the background-thin layer interfaces due to appearance of slow P-waves as the permeability of the background increases. To this end we estimate the corresponding slow P velocities $V_{PS}$ for a frequency $f$ of 6.3 Hz, assuming a low frequency approximation \cite{Norris1993}: $V_{PS} = \sqrt{\frac {2 \pi f \,i }{D}}$. These velocities are  $1.0 \times 10^{-3}$ m/s, $3.3 \times 10^{-2}$ m/s, $1\times 10^{0}$ m/s and $1.0 \times 10^{1}$ m/s for background permeabilities of $10^{-9}$ D, $10^{-6}$, $10^{-3}$ D and $10^{-1}$ D, respectively. For comparison we present the $V_{PS}$ for the sand beds, which are $2.2 \times 10^{1}$ m/s and $4.7 \times 10^{0}$ m/s for the water and gas-saturated sands, respectively. Notice that the $V_{PS}$ for a background permeability of $10^{-3}$ D is already of the same order of the $V_{PS}$ of the gas-saturated sand. This suggest that for this and higher permeabilities, the background behaves as poroelastic, implying that an additional energy partition at the background-thin layer interfaces should be considered for an accurate calculation of reflectivities. However, a viscoelastic representation of the thin layer, as the one  proposed in this study, cannot reproduce such energy partition at the interfaces with the background.
Nonetheless, background rocks of interest such as shale and crystalline rocks in their intact state can exhibit permeabilities in the range of nano ( $10^{-6}$) to micro ($10^{-9}$) Darcies  \cite<e.g.,>{Mitchell2012, Fisher2017, Wenning2018, Zhao2018} which, as suggested by the current analysis, can be considered as impermeable for the seismic frequencies. This, of course allows representing the background-porous thin layer by elastic and viscoelastic media, respectively, as shown in this study.
